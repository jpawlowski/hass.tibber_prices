...
- **Address the reader informally in every language**: German "du" (never "Sie"), Dutch "je/jouw" (never "u/uw"), Swedish and Norwegian "du". Matches Home Assistant's own translations, so a polite string reads as foreign inside the UI it appears in. Enforced by `tests/test_translation_tone.py`. Beware the near-misses that are *not* address: German "Sie wurde entfernt" and "ihr Zuhause" are third-person references to a feminine noun, and Dutch "u" is the abbreviation for "uur" — the guard's patterns are narrow on purpose. Where such a sentence reads like address at a glance, rephrase it anyway.
- **Never wrap a placeholder in ASCII single quotes**: the frontend renders through ICU MessageFormat, where `'` before a brace starts an escape — `'{name}'` reaches the user as the literal text `{name}`, unsubstituted. The backend uses `str.format()` and is unaffected, so the log looks right while the UI does not. Use typographic quotes (`„…“`, `“…”`, `«…»`). Enforced by `tests/test_translation_placeholders.py`; Hassfest also rejects it.
